...
const Meetups = () => {
  ...
  const updateUserDogsCancelation = (data) => {
    let spreadDogs = [...dogs];
    let dogToUpdateInvitor = spreadDogs.find(
      (dog) => dog.id === data.invitor.id
    );
    let dogToUpdateInvitee = spreadDogs.find(
      (dog) => dog.id === data.invitee.id
    );
    let unupdatedInvitorMeetups = dogToUpdateInvitor.sent_invitations.filter(
      (invite) => invite.id !== data.id
    );
    let unupdatedInviteeMeetups =
      dogToUpdateInvitee.recieved_invitations.filter(
        (invite) => invite.id !== data.id
      );
    let updatedInvitorDog = {
      ...dogToUpdateInvitor,
      sent_invitations: [...unupdatedInvitorMeetups, data],
    };
    let updatedInviteeDog = {
      ...dogToUpdateInvitee,
      recieved_invitations: [...unupdatedInviteeMeetups, data],
    };
    let unupdatedDogsWithoutInvitor = dogs.filter(
      (dog) => dog.id !== data.invitor.id
    );
    let unupdatedDogsWithoutInvitorAndInvitee =
      unupdatedDogsWithoutInvitor.filter((dog) => dog.id !== data.invitee.id);
    let updatedDogs = [
      ...unupdatedDogsWithoutInvitorAndInvitee,
      updatedInvitorDog,
      updatedInviteeDog,
    ];
    setDogs(updatedDogs);
